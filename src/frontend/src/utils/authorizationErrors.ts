export function normalizeAuthError(error: unknown): string {
  ...
  if (errorMessage.includes('Email not registered')) {
    return 'This email has not been registered. Please register your email first.';
  }
  ...
  if (errorMessage.includes('Email not yet associated')) {
    return 'This email has not been associated with any Internet Identity principal yet.';
  }
